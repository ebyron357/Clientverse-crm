...
import { Boxes, BriefcaseBusiness, CirclePlus, GitBranch, Search } from "lucide-react";
import { CLIENTVERSE_MODULES, MODULE_GROUPS, MODULE_STATES, PINNED_GROUPS } from "@/platform/modules";
...
export default function GlobalCommandDialog({ open, onOpenChange, onQuickCreate }) {
  ...
  const orderedGroups = [...PINNED_GROUPS, ...MODULE_GROUPS.filter((group) => !PINNED_GROUPS.includes(group))];
  ...
  return <CommandDialog open={open} onOpenChange={onOpenChange}>
    ...
    <CommandList className="cv-scrollbar max-h-[460px]">
      ...
      <CommandGroup heading="Top actions">
        <CommandItem value="create opportunity pipeline deal" onSelect={() => go("/pipeline")} data-testid="cmd-create-opportunity">
          <GitBranch className="size-4 text-primary" />
          <div className="flex flex-1 flex-col"><span className="font-medium">Create opportunity</span><span className="text-xs text-muted-foreground">Open pipeline and start a new deal</span></div>
        </CommandItem>
        <CommandItem value="workspaces client 360" onSelect={() => go("/workspaces")} data-testid="cmd-workspaces">
          <BriefcaseBusiness className="size-4 text-primary" />
          <div className="flex flex-1 flex-col"><span className="font-medium">Workspaces</span><span className="text-xs text-muted-foreground">Jump to Client 360 portfolio</span></div>
        </CommandItem>
        <CommandItem value="integrations registries providers" onSelect={() => go("/registries")} data-testid="cmd-integrations">
          <Boxes className="size-4 text-primary" />
          <div className="flex flex-1 flex-col"><span className="font-medium">Integrations</span><span className="text-xs text-muted-foreground">Provider registries and connection health</span></div>
        </CommandItem>
