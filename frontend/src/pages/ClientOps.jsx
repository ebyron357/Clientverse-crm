...
function WorkspacePicker({ workspaces, value, onChange }) { return <div className="min-w-[230px]"><Label className="sr-only">Active client workspace</Label><Select value={value} onValueChange={onChange}><SelectTrigger><SelectValue placeholder="Choose a workspace" /></SelectTrigger><SelectContent>{workspaces.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select></div>; }
