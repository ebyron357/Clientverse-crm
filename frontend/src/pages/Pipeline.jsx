...
import { ArrowRight, BriefcaseBusiness, Building2, CircleDollarSign, Filter, GitBranch, Plus, RefreshCw, Search, Sparkles, X } from "lucide-react";
...
export default function Pipeline() {
  ...
  const load = useCallback(async () => {
    ...
    try { const [opportunityResponse, companyResponse, workspaceResponse] = await Promise.all([api.get("/opportunities"), api.get("/companies"), api.get("/workspaces")]); setOpportunities(opportunityResponse.data); setCompanies(companyResponse.data); setWorkspaces(workspaceResponse.data || []); }
  ...
  return <div className="cv-page">
    ...
    <section aria-label="Revenue pipeline by stage" className="cv-scrollbar overflow-x-auto pb-2"><div className="grid min-w-[1100px] grid-cols-5 gap-4">{STAGES.map((stage) => <StageColumn key={stage.key} stage={stage} opportunities={filtered.filter((opportunity) => opportunity.stage === stage.key)} companyName={companyName} workspaces={workspaces} onMove={move} navigate={navigate} />)}</div></section>
...
function OpportunityCard({ opportunity, companyName, workspaces, onMove, navigate }) {
  const advanceTo = nextStage(opportunity.stage);
  const linkedWorkspace = (workspaces || []).find((ws) => ws.opportunity_id === opportunity.id || (opportunity.company_id && ws.company_id === opportunity.company_id && opportunity.stage === "closed_won"));
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-[0_1px_2px_rgba(10,22,40,.03)] transition-shadow hover:shadow-[0_8px_18px_rgba(10,22,40,.08)]" data-testid={`opportunity-card-${opportunity.id}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-sm font-bold text-[#132038]">{opportunity.name}</h3>
          <p className="mt-1 truncate text-xs text-slate-500">{companyName(opportunity.company_id)}</p>
        </div>
        <span className="shrink-0 font-display text-sm font-bold text-[#0a1628]">{money(opportunity.value)}</span>
      </div>
      <div className="mt-3 flex flex-wrap gap-1.5 border-t border-slate-100 pt-3">
        {advanceTo && (
          <Button size="sm" className="h-7 cv-action-primary text-xs" onClick={() => onMove(opportunity, advanceTo)} data-testid={`advance-${opportunity.id}`}>
            Advance <ArrowRight className="ml-1 h-3 w-3" />
          </Button>
        )}
        {opportunity.company_id && (
          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => navigate(`/companies`)} data-testid={`open-company-${opportunity.id}`}>
            <Building2 className="mr-1 h-3 w-3" />Company
          </Button>
        )}
        {linkedWorkspace && (
          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => navigate(`/workspaces/${linkedWorkspace.id}`)} data-testid={`open-workspace-${opportunity.id}`}>
            <BriefcaseBusiness className="mr-1 h-3 w-3" />Workspace
          </Button>
        )}
      </div>
      <div className="mt-2">
        <Select value={opportunity.stage} onValueChange={(value) => onMove(opportunity, value)}>
          <SelectTrigger className="h-8 w-full border-slate-200 bg-slate-50 text-xs font-medium text-slate-600"><SelectValue /></SelectTrigger>
          <SelectContent>{STAGES.map((stage) => <SelectItem key={stage.key} value={stage.key}>{stage.label}</SelectItem>)}<SelectItem value="closed_lost">Closed lost</SelectItem></SelectContent>
        </Select>
      </div>
    </article>
  );
