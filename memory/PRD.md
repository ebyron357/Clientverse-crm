...
- Reliability guards: `backend/tests/test_module_structure.py` (5 tests) AND a standalone `backend/scripts/validate_app_structure.py` (exits non-zero on structural failure) — both fail on syntax corruption, import/circular failure, business-logic creeping back into `server.py`, duplicate route registration, duplicate router inclusion, or missing routers. Also added repo-root `AGENTS.md` instructing future agents where to add routes/services (never grow `server.py`).
