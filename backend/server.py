...
async def _upsert_alert(tenant_id, workspace_id, atype, severity, source, summary, source_ref):
    ...
    if existing:
        ...
        return False
    await db.alerts.insert_one({"id": new_id("alert"), "tenant_id": tenant_id, "workspace_id": workspace_id,
...
async def _resolve_alerts(tenant_id, atype, source_ref):
    await db.alerts.update_many({"tenant_id": tenant_id, "type": atype, "source_ref": source_ref,
